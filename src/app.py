...
from src.features.pdf_qa import ask_pdf, load_pdf, build_pdf_qa_graph
...
/* Force teal on primary buttons */
button.primary,
.primary-btn {
    background: linear-gradient(135deg, #0D9488, #0c0d0d) !important;
...
.tab-nav button.selected { background: linear-gradient(135deg, #6C5CE7, #a29bfe) !important; color: white !important; box-shadow: 0 4px 12px rgba(108,92,231,0.3) !important; }
...
textarea:focus, input:focus { border-color: var(--primary) !important; box-shadow: 0 0 0 3px rgba(108,92,231,0.15) !important; }

/* Buttons */
.primary-btn { background: linear-gradient(135deg, #6C5CE7, #a29bfe) !important; border: none !important; border-radius: var(--radius-sm) !important; color: white !important; font-weight: 600 !important; padding: 12px 28px !important; font-size: 14px !important; box-shadow: 0 4px 12px rgba(108,92,231,0.3) !important; transition: all 0.2s !important; }
.primary-btn:hover { transform: translateY(-1px) !important; box-shadow: 0 6px 20px rgba(108,92,231,0.4) !important; }
...
/* Labels */
label span { font-weight: 600 !important; color: var(--text) !important; font-size: 13px !important; letter-spacing: 0.3px !important; }

/* Chat */
.chatbot { border-radius: var(--radius) !important; border: 1px solid var(--border) !important; box-shadow: var(--shadow) !important; }
.message.user { background: linear-gradient(135deg, #6C5CE7, #a29bfe) !important; color: white !important; border-radius: 18px 18px 4px 18px !important; }
.message.bot { background: white !important; border: 1px solid var(--border) !important; border-radius: 18px 18px 18px 4px !important; }
...
/* Stat badges */
.badge { display: inline-block; background: var(--surface-2); border: 1px solid var(--border); border-radius: 20px; padding: 4px 14px; font-size: 12px; color: var(--primary); font-weight: 600; margin: 4px; }
...
def create_app():
    with gr.Blocks(title="GenAI Toolkit",
            ...
            with gr.Tab("🌐 Translation"):
                ...
                with gr.Row(equal_height=True):
                    with gr.Column(scale=1, elem_classes="feature-card"):
                        ...
                        t_btn = gr.Button("Translate ➜", variant="primary", elem_classes="primary-btn")
                    with gr.Column(scale=1, elem_classes="feature-card"):
                        t_out = gr.Textbox(label="Translation", lines=8, interactive=False, elem_classes="output-box")
                        gr.HTML('<p style="font-size:12px;color:#8b8b9e;margin-top:8px">⚡ Powered by LLaMA 3.1 · Fast & accurate</p>')
            ...
            with gr.Tab("📝 Summarize"):
                ...
                with gr.Row(equal_height=True):
                    with gr.Column(scale=1, elem_classes="feature-card"):
                        ...
                        s_btn = gr.Button("Summarize ✦", variant="primary", elem_classes="primary-btn")
                    with gr.Column(scale=1, elem_classes="feature-card"):
                        s_out = gr.Textbox(label="Summary", lines=12, interactive=False, elem_classes="output-box")
                        gr.HTML('<p style="font-size:12px;color:#8b8b9e;margin-top:8px">📊 Supports long documents via map-reduce chunking</p>')
            ...
            with gr.Tab("🔍 Keywords"):
                ...
                with gr.Row(equal_height=True):
                    with gr.Column(scale=1, elem_classes="feature-card"):
                        ...
                        k_btn = gr.Button("Extract Keywords 🔍", variant="primary", elem_classes="primary-btn")
                    with gr.Column(scale=1, elem_classes="feature-card"):
                        k_out = gr.Textbox(label="Extracted Keywords", lines=10, interactive=False, elem_classes="output-box")
                        gr.HTML('<p style="font-size:12px;color:#8b8b9e;margin-top:8px">🏷️ Returns keywords with context and relevance</p>')
                    ...
                    with gr.Tab("📧 Email"):
                        with gr.Row(equal_height=True):
                            with gr.Column(scale=1, elem_classes="feature-card"):
                                ...
                                e_btn = gr.Button("Generate Email ✉️", variant="primary", elem_classes="primary-btn")
                            with gr.Column(scale=1, elem_classes="feature-card"):
                    ...
                    with gr.Tab("📄 Cover Letter"):
                        with gr.Row(equal_height=True):
                            with gr.Column(scale=1, elem_classes="feature-card"):
                                ...
                                cl_btn  = gr.Button("Generate Cover Letter 📄", variant="primary", elem_classes="primary-btn")
                            with gr.Column(scale=1, elem_classes="feature-card"):
            ...
            with gr.Tab("💬 Chat"):
                ...
                with gr.Column(elem_classes="feature-card"):
                    ...
                    with gr.Row():
                        ...
                        gr.HTML('<p style="font-size:12px;color:#8b8b9e;padding-top:10px">🧠 Maintains full conversation context via LangGraph state</p>')
            ...
            with gr.Tab("📄 PDF Q&A"):
                ...
                with gr.Row(equal_height=True):
                    with gr.Column(scale=1, elem_classes="feature-card"):
                        ...
                        p_btn    = gr.Button("Ask Question 🔍", variant="primary", elem_classes="primary-btn")
                        gr.HTML('<p style="font-size:12px;color:#8b8b9e;margin-top:8px">📚 Uses FAISS vector search + semantic chunking</p>')
                    with gr.Column(scale=1, elem_classes="feature-card"):
                ...
                def answer_pdf_question(question, pdf_text):
                    ...
                    result = graph.invoke({"pdf_text": pdf_text, "question": question, "context": "", "answer": ""})
        ...
        gr.HTML("""
        <div style="text-align:center;padding:2rem;color:#8b8b9e;font-size:13px;margin-top:1rem">
            Built with LangChain · LangGraph · Groq · Gradio &nbsp;|&nbsp; 
            <a href="https://github.com/shashankheg" style="color:#6C5CE7;text-decoration:none">GitHub</a>
        </div>
        """)
